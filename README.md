...
| `search <query>` | Search for notes with parent-context metadata; readable `tags` remain optional and SDK-limited |
| `search-tag <tag>` | Search for tagged notes with ancestor context |
| `read <rem-id>` | Read a note by ID with markdown or structured content; readable `tags` remain SDK-limited |
